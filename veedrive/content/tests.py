...
class TestPath(unittest.TestCase):
    ...
    def test_exceptions(self):
        ...
        with self.assertRaises(PermissionError):
            fs_manager.validate_path(config.SANDBOX_PATH + "../../", "dir")
        with self.assertRaises(Exception):
            fs_manager.validate_path(config.SANDBOX_PATH + "../", "file")
        with self.assertRaises(Exception):
            fs_manager.validate_path(config.SANDBOX_PATH + "folder1", "file")
...
class TestWsHandlers(unittest.TestCase):
    ...
    def test_create_file_response(self):
        ...
        assert obj["original"] == f"{config.CONTENT_URL}/{path}"
